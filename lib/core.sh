...
install_launch_agent(){ step "安装开机自启"; mkdir -p "$(dirname "$LAUNCH_AGENT_PLIST")"; local oc_bin; oc_bin=$(command -v openclaw || true); [[ -n "$oc_bin" ]] || die "未找到 openclaw 命令"; cat > "$LAUNCH_AGENT_PLIST" <<EOF
